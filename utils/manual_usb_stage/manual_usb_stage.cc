...
int main(int argc, char *argv[])
{
	...
	for (axis = 0; axis < PI_USB_MAX_CONTROLLERS; axis++) {
		...
		if (access(device, F_OK) == 0) {
			if ((ret = pi_usb_open(device)) < 0) {
				fprintf(stderr,
					"Error in pi_usb_open, quitting with exit value %d\n",
					ret);
				return ret;
			}
	...
	for (axis = 0; axis < n; axis++) {
		if (restore_position == 1)
			pi_usb_move_absolute_real(axis, initial_pos[axis],
						  PI_USB_WAIT);
...
int move_interactive(float initial_pos[PI_USB_MAX_CONTROLLERS], int n)
{
	...
	if (n > 0)
		cout << "Use normal left-right arrow keys to move axis 0" <<
		    "\r\n";
	if (n > 1)
		cout << "Use normal up-down arrow keys to move axis 1" <<
		    "\r\n";
	if (n > 2)
		cout <<
		    "Use keypad left-right arrow keys with NumLock ON to move axis 2"
		    << "\r\n";
	if (n > 3)
		cout <<
		    "Use keypad up-down arrow keys with NumLock ON to move axis 3"
		    << "\r\n" << flush;
	...
	for (axis = 0; axis < n; axis++) {
		...
		if (pi_usb_is_rotation_stage(axis) == 1) {
			...
			cout << "Axis " << axis <<
			    " is a rotation stage, default move size set to " <<
			    move_size[axis] << " deg" << "\r\n" << flush;
		} else {
			is_rot[axis] = 0;
			msi[axis] = 3;
			...
			cout << "Axis " << axis <<
			    " is a linear stage, default move size set to " <<
			    move_size[axis] << " um" << "\r\n" << flush;
		}
	}

	for (axis = 0; axis < n; axis++) {
		if ((is_rot[axis] == 1) && (msi[axis] < 3))
			last_pos[axis] = pi_usb_get_pos_real(axis);
		else
			last_pos[axis] = roundf(pi_usb_get_pos_real(axis));
...
// 
		case KEY_UP:	// these definitions are from curses.h
			if (n > 1)
				pi_usb_move_relative_real(1, move_size[1],
							  PI_USB_WAIT);
			break;
		case KEY_DOWN:
			if (n > 1)
		...
		case KEY_LEFT:
			if (n > 0)
				pi_usb_move_relative_real(0, -move_size[0],
							  PI_USB_WAIT);
			break;
		case KEY_RIGHT:
			if (n > 0)
		...
		case DET_UP_CHILLI:	// for when num lock hard maps the keypad (see also note below)
			if ((n > 3) && (move_size[3] >= 10000)
			    && (idiot_mode == 1) && (is_rot[3] == 0)) {
				cout <<
				    "Oi! Sharples! No!\r\nReduce your step size before moving the detector stages!\r\n";
				cout <<
				    "Press 'I' to turn off idiot mode, this will allow you\r\nto crash the stages much more easily.\r\n";
				break;
			}
			if (n > 3)
				pi_usb_move_relative_real(3, move_size[3],
							  PI_USB_WAIT);
			break;
		...
		case DET_LEFT_CHILLI:
			if ((n > 2) && (move_size[2] >= 10000)
			    && (idiot_mode == 1) && (is_rot[2] == 0)) {
				cout <<
				    "Oi! Sharples! No!\r\nReduce your step size before moving the detector stages!\r\n";
				cout <<
				    "Press 'I' to turn off idiot mode, this will allow you\r\nto crash the stages much more easily.\r\n";
			...
			if (n > 2)
				pi_usb_move_relative_real(2, -move_size[2],
							  PI_USB_WAIT);
			break;
		...
		case FASTER:
			cout <<
			    "Increase move size (\"faster\"): 'l' for linear stages, 'r' for rotation: "
			    << flush;
			echo();
			kp = getch();
			cout << "\r" << endl;
				...
				for (axis = 0; axis < n; axis++) {
					if (is_rot[axis] == 1) {
						if (msi[axis] < 6)
							msi[axis]++;
						move_size[axis] =
						    rot_move_size[msi[axis]];
						cout << "\t" << move_size[axis]
						    <<
						    " degrees/keypress on axis "
						    << axis << "\r\n" << flush;
					}
				}
				break;
				...
				for (axis = 0; axis < n; axis++) {
					if (is_rot[axis] == 0) {
						if (msi[axis] < 5)
							msi[axis]++;
						move_size[axis] =
						    lin_move_size[msi[axis]];
						cout << "\t" << move_size[axis]
						    <<
						    " microns/keypress on axis "
						    << axis << "\r\n" << flush;
					}
				}
				break;
		...
		case SLOWER:
			cout <<
			    "Decrease move size (\"slower\"): 'l' for linear stages, 'r' for rotation: "
			    << flush;
			echo();
			kp = getch();
			cout << "\r" << endl;
		...
		case ORIGIN:
			cout <<
			    "Set origin: 'd' to set detector origin (axes 2 & 3),\r\n"
			    << flush;
			cout <<
			    "            's' to set sample origin (axes 0 & 1),\r\n"
			    << flush;
			cout <<
			    "            'a' for all axes, '0' to 'F' (caps A-F) for those axes,\r\n"
			    << flush;
			cout <<
			    "            any other key does not set the origin.... : "
			    << flush;
			echo();
			kp = getch();
			cout << "\r\t" << endl;
			...
			case '0':
				if (n > 0) {
					pi_usb_set_origin(0);
					cout << "Origin set on axis 0 (sample)"
					    << "\r\n";
				}
				break;
			case '1':
				if (n > 1) {
					pi_usb_set_origin(1);
					cout << "Origin set on axis 1 (sample)"
					    << "\r\n";
				}
				break;
			case '2':
			...
			case '9':
				if (n > 9) {
					pi_usb_set_origin(9);
					cout << "Origin set on axis 9" <<
					    "\r\n";
				}
				break;
			case 'A':
				if (n > 10) {
					pi_usb_set_origin(10);
					cout << "Origin set on axis 10" <<
					    "\r\n";
				}
				break;
			case 'B':
				if (n > 11) {
					pi_usb_set_origin(11);
					cout << "Origin set on axis 11" <<
					    "\r\n";
				}
				break;
			case 'C':
				if (n > 12) {
					pi_usb_set_origin(12);
					cout << "Origin set on axis 12" <<
					    "\r\n";
				}
				break;
			case 'D':
				if (n > 13) {
					pi_usb_set_origin(13);
					cout << "Origin set on axis 13" <<
					    "\r\n";
				}
				break;
			case 'E':
				if (n > 14) {
					pi_usb_set_origin(14);
					cout << "Origin set on axis 14" <<
					    "\r\n";
				}
				break;
			case 'F':
				if (n > 15) {
					pi_usb_set_origin(15);
					cout << "Origin set on axis 15" <<
					    "\r\n";
				}
				break;
			default:
...
//
		case VELOCITY:
			cout <<
			    "Input axis to set velocity on (enter 0-9 or A-F): "
			    << flush;
			echo();
			kp = getch();
			switch (kp) {
			...
			echo();
			if (is_rot[axis] == 0)
				cout << "\r\nInput velocity (in microns/sec): "
				    << flush;
			else
				cout << "\r\nInput velocity (in degrees/sec): "
				    << flush;
			getnstr(inptvalue, 10);
			if (n > axis)
				pi_usb_set_vel_real(axis,
						    strtof(inptvalue, NULL));
			noecho();
			break;

		case MANUAL:
			cout << "Input axis to move (enter 0,1,2 or 3): " <<
			    flush;
			echo();
			kp = getch();
			switch (kp) {
			...
			if (is_rot[axis] == 0)
				cout <<
				    "\r\nInput relative distance in microns: "
				    << flush;
			else
				cout <<
				    "\r\nInput relative distance in degrees: "
				    << flush;
			getnstr(inptvalue, 10);
			if (n > axis)
				pi_usb_move_relative_real(axis,
							  strtof(inptvalue,
								 NULL),
							  PI_USB_WAIT);
			noecho();
		...
		case GOHOME:
			cout << "All connected axes going home to (0,0,0,0,...)"
			    << "\r\n";
			for (axis = 0; axis < n; axis++)
				pi_usb_move_absolute(axis, 0, PI_USB_WAIT);
			break;

		case IDIOT_OFF:
			cout <<
			    "Idiot mode is now turned off, you will now\r\n";
			cout <<
			    "be able to move the detector stages by upto\r\n";
			cout <<
			    "10mm at a time (rather than 1mm)... be careful!\r\n";
			idiot_mode = 0;
			break;
		case IDIOT_ON:
			cout << "Idiot mode is now turned on, you will now\r\n";
			cout <<
			    "NOT be able to move the detector stages by any\r\n";
		...
		case HELP:
			...
			cout << "Make sure Caps Lock is OFF and Num Lock is ON"
			    << "\r\n" << endl;
			cout <<
			    "Axes 2 and 3, use arrow keys on number pad, with Num Lock ON"
			    << "\r\n";
			cout <<
			    "Axes 0 and 1, use normal arrow keys (left/right = axis 0)"
			    << "\r\n" << endl;
...
//                      }
		...
		for (axis = 0; axis < n; axis++) {
			...
			if ((limit_status == PI_USB_ON_POSITIVE_LIMIT)
			    || (limit_status == PI_USB_ON_NEGATIVE_LIMIT)) {
				cout << "LIMIT SWITCH reached on axis " << axis
				    << " at " << pi_usb_get_pos_real(axis) <<
				    ", moving back to " << last_pos[axis] <<
				    "\r\n" << flush;
				pi_usb_move_absolute_real(axis, last_pos[axis],
							  PI_USB_WAIT);
			}
		}
		...
		for (axis = 0; axis < n; axis++) {
			if ((is_rot[axis] == 1) && (msi[axis] < 3))
				...
				last_pos[axis] =
				    roundf(pi_usb_get_pos_real(axis));
			cout << "a" << axis << ": " << last_pos[axis] << " ";
		}
		cout << "\r\n" << flush;
