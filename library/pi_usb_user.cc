...
{
	return serial_receive(PI_USB_FD[axis], buf, buf_len, (const char)3,
			      (const char)'\r');
}
...
BOOL pi_usb_recall_all_axes_pos_real(BOOL * found_axis, float *pos, BOOL silent)
{
	...
	if (fi < 0) {
		printf
		    ("Recalling USB stage position: %s can't be opened (bad)\n",
		     PI_USB_POSITION_FILE);
		fclose(fi);
		return FALSE;
	}
			     ...
			     BOOL silent)
{
	...
	if (found_axis[axis] == FALSE) {
		...
		if (silent == FALSE) {
			printf
			    ("Recalling USB stage positions: axis %d is at %.1f\n",
			     axis, saved_pos);
			printf
			    ("According to the controller  : axis %d is at %.1f\n",
			     axis, current_pos);
		}
		if (fabs(current_pos - saved_pos) <= max_discrepancy) {
			...
			if (interactive == FALSE) {
				...
				do {
					printf
					    ("the (f)ile or the (c)ontroller? ");
					fgets(response, 9, stdin);
				} while ((response[0] != 'f')
					 && (response[0] != 'c'));
				if (response[0] == 'f') {
					printf
					    ("Ok then, restoring position from the file\n");
					ret_pos = saved_pos;
				} else {
					printf
					    ("Ok then, using the position according to the controller\n");
					ret_pos = current_pos;
				}
			}
		}
...
BOOL pi_usb_save_pos_real(int axis, BOOL silent)
{
	...
	if (out < 0) {
		printf
		    ("Saving USB stage position: %s can't be opened for writing (bad)\n",
		     PI_USB_POSITION_FILE);
		return FALSE;
	}
...
BOOL pi_usb_recall_installed_stage(int axis, char *stage_type, BOOL silent)
{
	...
	if (access(PI_USB_INSTALLED_STAGES_FILE, F_OK) != 0) {
		printf
		    ("Recalling USB installed stages: %s doesn't exist (bad)\n",
	...
	if (access(PI_USB_INSTALLED_STAGES_FILE, R_OK) != 0) {
		printf
		    ("Recalling USB installed stages: %s can't be read (bad)\n",
		     PI_USB_INSTALLED_STAGES_FILE);
		return FALSE;
	}
	...
	if (fi < 0) {
		printf
		    ("Recalling USB installed stages: %s can't be opened (bad)\n",
		     PI_USB_INSTALLED_STAGES_FILE);
		fclose(fi);
		return FALSE;
	}
...
{
	pi_usb_set_trigger_increment(axis,
				     pi_usb_real_to_count(axis,
							  increment_real));
}
